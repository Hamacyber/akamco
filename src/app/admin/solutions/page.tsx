...
// ─── image upload helper ─────────────────────────────────────────
function ImageUpload({ value, onChange, folder = 'solutions' }: { value: string; onChange: (v: string) => void; folder?: string }) {
  ...
  return (
    <div>
      <div className="flex gap-2">
        <input className={inputCls + ' flex-1'} value={value} onChange={e => onChange(e.target.value)} placeholder="/images/solutions/... or upload →" />
...
function emptySolution(): Partial<SolutionDetail> & { heroStat: { value: string; label: string } } {
  return {
    id: '', title: '', tagline: '', description: '', image: '',
    heroStat: { value: '', label: '' },
    gradient: 'from-blue-500 to-indigo-600', icon: '',
...
export default function AdminSolutionsPage() {
  ...
  function openEdit(s: SolutionDetail) {
    setEditingId(s.id);
    setForm({
      ...emptySolution(),
      ...s,
  ...
  async function save() {
    ...
    if (!editingId && !form.id?.trim()) { setFormError('ID (slug) is required for new solutions'); return; }
    ...
    try {
      ...
      if (data.pending) {
        ...
        setToast({ msg: 'Solution saved successfully.', type: 'success' });
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="sticky top-0 z-30 flex items-center justify-between border-b border-gray-800 bg-gray-950/90 backdrop-blur px-6 py-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{solutions.length} solution{solutions.length !== 1 && 's'}</p>
        </div>
      ...
      <div className="px-6 py-4">
        <input
          ...
          placeholder="Search solutions…" value={search} onChange={e => setSearch(e.target.value)}
        />
      </div>
      ...
      <div className="px-6 pb-12">
        {loading ? (
          ...
          <p className="text-gray-500 text-sm">No solutions found.</p>
        ) : (
        ...
        <div className="fixed inset-0 z-40 flex">
          ...
          <aside className="w-full max-w-2xl bg-gray-900 border-l border-gray-700 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-gray-800 px-6 py-4 shrink-0">
              <h2 className="text-base font-bold">{editingId ? 'Edit Solution' : 'Add Solution'}</h2>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input className={inputCls} value={form.title ?? ''} onChange={e => setField('title', e.target.value)} placeholder="e.g. Cloud Infrastructure" />
                </div>
                <div>
                  <label className={labelCls}>ID / Slug {!editingId && '*'}</label>
                  <input className={inputCls} value={form.id ?? ''} onChange={e => setField('id', e.target.value)} placeholder="e.g. cloud-infrastructure" disabled={!!editingId} />
              ...
              <div>
                ...
                <ImageUpload value={form.image ?? ''} onChange={v => setField('image', v)} folder="solutions" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={labelCls}>Hero Stat — Value</label>
                  <input className={inputCls} value={form.heroStat?.value ?? ''} onChange={e => setField('heroStat', { ...form.heroStat, value: e.target.value })} placeholder="e.g. 50%" />
                </div>
                <div>
                  <label className={labelCls}>Hero Stat — Label</label>
                  <input className={inputCls} value={form.heroStat?.label ?? ''} onChange={e => setField('heroStat', { ...form.heroStat, label: e.target.value })} placeholder="e.g. Cost Reduction" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input className={inputCls} value={form.gradient ?? ''} onChange={e => setField('gradient', e.target.value)} placeholder="from-blue-500 to-indigo-600" />
                </div>
                <div>
            ...
            <div className="shrink-0 border-t border-gray-800 px-6 py-4 flex items-center justify-end gap-3">
              ...
              <button onClick={save} disabled={saving} className="rounded-lg bg-[#00C46A] px-5 py-2 text-sm font-semibold text-black hover:bg-green-400 disabled:opacity-40 transition-colors">
                {saving ? 'Saving…' : editingId ? 'Save Changes' : 'Add Solution'}
              </button>
            </div>
          </aside>
        </div>
