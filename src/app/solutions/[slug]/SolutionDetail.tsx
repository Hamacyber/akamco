...
import type { SolutionDetail as SolutionDetailType } from '@/data/solutions';
import { iconMap } from '@/components/ui/Icons';

interface Props {
  solution: SolutionDetailType;
}
...
export default function SolutionDetail({ solution }: Props) {
  ...
  return (
    <div className={`relative z-10 ${dark ? 'text-white' : 'text-gray-900'}`}>
      ...
      <div className="relative min-h-[540px] flex flex-col lg:flex-row pt-20">
        ...
        <div className={`relative z-10 flex flex-col justify-center px-5 md:px-14 xl:px-20 py-12 lg:w-[52%] ${dark ? 'bg-[#0B0F14]' : 'bg-gray-50'}`}>
          <div className="flex items-center gap-1.5 text-xs font-medium mb-8">
            ...
            <Link href="/solutions/datacenter-solutions" className={`${dark ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'} transition-colors`}>Solutions</Link>
            <span className={dark ? 'text-gray-700' : 'text-gray-300'}>/</span>
          ...
          <div className="mb-6 flex items-center gap-3">
            <div className="w-14 h-14 rounded-2xl bg-accent flex items-center justify-center text-white shadow-lg shadow-accent/30">
              {SolutionIcon && <SolutionIcon className="w-7 h-7" />}
            </div>
            <span className={`text-xs font-bold uppercase tracking-widest ${dark ? 'text-gray-400' : 'text-gray-400'}`}>Solution</span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Link href={`/blog?service=${solution.id}`} className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm border transition-colors ${dark ? 'border-gray-700 text-gray-300 hover:border-gray-500' : 'border-gray-200 text-gray-600 hover:border-gray-400'}`}>
              Projects
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 xl:px-16 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-5">
            <p className="text-xs font-bold uppercase tracking-widest text-accent">About This Solution</p>
            {solution.longDescription.map((para, i) => (
      ...
      {/* FEATURES â€” 2-col left-border cards */}
      ...
      <section className={`py-14 ${dark ? 'bg-black/20' : 'bg-gray-50/80'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 xl:px-16">
          ...
          <h2 className={`text-2xl font-black mb-8 ${dark ? 'text-white' : 'text-gray-900'}`}>Who Is This Solution For?</h2>
          <div className="flex flex-wrap gap-3">
            {solution.forWhom.map((who, i) => (
              <span
                key={i}
        ...
        <div className="rounded-3xl overflow-hidden relative">
          ...
          <div className="relative px-10 py-14 md:px-16 flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-black text-white mb-2">Ready to deploy {solution.title}?</h2>
      ...
      {/* OTHER SOLUTIONS â€” text link strip */}
      <section className={`border-t ${dark ? 'border-gray-800 bg-black/20' : 'border-gray-100 bg-gray-50/60'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 xl:px-16 py-12">
          <p className={`text-xs font-bold uppercase tracking-widest mb-6 ${dark ? 'text-gray-500' : 'text-gray-400'}`}>Other Solutions</p>
